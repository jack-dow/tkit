...
function OrganizationLogoImage({ setUploadedLogoImage }: { setUploadedLogoImage: (file: File | null) => void }) {
	...
	const onDrop = React.useCallback(
		(acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
			acceptedFiles.forEach((file) => {
				setUploadedLogoImage(file);

				form.setValue("logoImageUrl", URL.createObjectURL(file), { shouldDirty: true });
			});
		...
		[form, toast, setUploadedLogoImage],
	);
	...
	const { getRootProps, getInputProps, isDragActive, open } = useDropzone({
		onDrop,
		noClick: logoImageUrl ? true : false,
		maxFiles: 1,
		accept: {
			"image/png": [".png"],
			"image/jpeg": [".jpeg", ".jpg"],
	...
	return (
		<div className="grid grid-cols-1 gap-2 xl:grid-cols-3 xl:gap-8 xl:gap-x-24">
			<div>
				<h2 className="text-base font-semibold leading-7 text-foreground">Logo Image</h2>
				<p className="text-sm leading-6 text-muted-foreground">
					This is the image that will be displayed on emails and invoices sent on your organization&apos;s behalf.
				</p>
			</div>
			{!logoImageUrl ? (
				<div className=" xl:col-span-2">
					<div>
						<div
							...
							className="relative block w-full cursor-pointer rounded-lg border-2 border-dashed border-zinc-300 p-12 text-center hover:border-zinc-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
						>
							<input {...getInputProps()} />
