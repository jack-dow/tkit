...
function VetClinicToVetRelationships({ isNew, variant }: { isNew: boolean; variant: "sheet" | "form" }) {
	...
	async function addVetClinicToVetRelationship(
		vet: ManageVetClinicFormSchema["vetToVetClinicRelationships"][number]["vet"],
	) {
		const relationship = {
			id: generateId(),
			...
			<FieldsWrapper title="Vets" description="Manage the relationships between this vet clinic and their vets.">
				<ManageVetSheet
					...
					onSuccessfulSubmit={(data) => {
						searchVetsInputRef?.current?.focus();
						void addVetClinicToVetRelationship({
							id: data.id,
				...
				<div className="sm:col-span-6">
					<MultiSelectSearchCombobox
						...
						onSelect={async (vet) => {
							...
							await addVetClinicToVetRelationship(vet);
						}}
